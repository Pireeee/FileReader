...
public class Squarify {
	...
	public Squarify(ArrayList<Float> sizes, ArrayList<String> ids, float x, float y, float width, float height) {
		data = new SquarifyData(sizes, ids, width, height);
	...
	private ArrayList<SquarifyRect> layoutRow(List<DataPoint> values, float x, float y, float dx, float dy) {
		...
		for (int i = 0; i < values.size(); i++) {
			rects.add(new SquarifyRect(x, y, width, values.get(i).getNormalizedValue() / width,
					values.get(i).getId(), values.get(i).getValue()));
